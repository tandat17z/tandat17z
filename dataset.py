...
class News20Dataset(Dataset):
    ...
    def transform(self, text):
        # encode document
        doc = [
            [self.vocab.index(word) if word in self.vocab else 1 
                            for word in word_tokenize(text=sent)]
            for sent in sent_tokenize(text=text)]  # if len(sent) > 0 -- phân tách văn bản thành các câu riêng biệt.
        
        # Mỗi câu dài tối đa Max_sent_length và 1 doc có tối đa max_doc_length
